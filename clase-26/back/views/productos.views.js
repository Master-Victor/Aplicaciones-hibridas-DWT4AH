function createProductListPage(productos) {
  ...
  for (let i = 0; i < productos.length; i++) {
    html +=
      "<li>" +
      `<a href="/productos/${productos[i]._id}">${productos[i].name} </a>` +
      `<a href="/productos/editar/${productos[i]._id}">Editar</a>` +
      `<a href="/productos/eliminar/${productos[i]._id}">Eliminar</a>`
      "</li>";
  }
  html += "</ul>";
