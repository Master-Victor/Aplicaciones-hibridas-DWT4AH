...
const editProducto = (req, res) =>{
  ...
  service.editProducto(id, producto)
    .then( (productoEditado) => {
      ...
      if(productoEditado){
        res.send( view.createPage("Producto Modificado", `<h2>Producto #${productoEditado._id} Editado con exito</h2>`) )
      }else{
...
const eliminarProducto = (req, res) => {
  ...
  .then( (productoEliminado) => {
    if(productoEliminado){
      res.send( view.createPage("Producto Eliminado", `<h2>Producto #${productoEliminado._id} Eliminado con exito</h2>`) )
    }else{
