...
// function createListProductos(productos){
//     let html = ""
...
const server = http.createServer( (req, res) => {
    ...
    if( req.url === "/" ){
        res.write(pages.createPage("Home", "<p>Victor</p>"))
    } 
    if( req.url === "/profesor" ){
        res.write(pages.createPage("Profesor", "<p>Victor Villafañe</p>"))
    } 
    if( req.url === "/materia" ){
        res.write(createPage("Materia", "<p>Aplicaciones Hibridas</p>"))
    } 
    if( req.url === "/productos" ){
        res.write(pages.createPage("Productos", pages.createListProductos(PRODUCTOS)))
    }
    res.end()
} )
